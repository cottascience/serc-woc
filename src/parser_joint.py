...
class Data:
    def __init__(self, path, device, y_lab):
        ...
        self.y_lab = y_lab
        if y_lab == "JOINTP":
            self.y_dict = [budget, schedule, objs]
            self.y_lab = ["PROJS", "PROJT", "PROJP"]
        else:
            self.y_dict = [RR]*10
            self.y_lab = ["RR1", "RR2", "RR3", "RR4", "RR5", "RR6", "RR7", "RR8", "RR9", "RR10"]
            ...
            if ex in ids:
                for i in range(len(self.instructor)):
                    if int(self.instructor[i]["Week"]) == (project_week[0] + 1) and self.instructor[i]["Project"] == project_week[1]:
                        if split == "train":
                            tmp = []
                            for d,l in zip(self.y_dict,self.y_lab):
                                tmp.append(d[self.instructor[i][l]])
                            self.y.append(tmp)
                        elif split == "test":
                            tmp = []
                            for d,l in zip(self.y_dict,self.y_lab):
                                tmp.append(d[self.instructor[i][l]])
                            self.test_y.append(tmp)
                        else:
                            tmp = []
                            for d,l in zip(self.y_dict,self.y_lab):
                                tmp.append(d[self.instructor[i][l]])
                            self.val_y.append(tmp)
                        label_exists = True
                if label_exists:
                    for i in range(len(self.instructor)):
                        if int(self.instructor[i]["Week"]) == (project_week[0]) and self.instructor[i]["Project"] == project_week[1]:
                            if split == "train":
                                ...
                                for d,l in zip(self.y_dict,self.y_lab):
                                    tmp.append(d[self.instructor[i][l]])
                                self.prev_y.append(tmp)
                            elif split == "test":
                                tmp = []
                                for d,l in zip(self.y_dict,self.y_lab):
                                    tmp.append(d[self.instructor[i][l]])
                                self.test_prev_y.append(tmp)
                            else:
                                tmp = []
                                for d,l in zip(self.y_dict,self.y_lab):
                                    tmp.append(d[self.instructor[i][l]])
                                self.val_prev_y.append(tmp)
